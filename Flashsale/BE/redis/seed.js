/**
 * Data Grid Seed — nạp dữ liệu sản phẩm + tồn kho vào Redis
 * Chạy 1 lần trước khi khởi động các PU:  node redis/seed.js
 */
...
async function seed() {
  console.log('🌱 Seeding Data Grid (Redis)...');

  // Xoá data cũ
  await redis.del('products');
  const stockKeys = products.map(p => `stock:${p.id}`);
  if (stockKeys.length) await redis.del(...stockKeys);

  const pipeline = redis.pipeline();

  for (const p of products) {
    // Lưu product dưới dạng JSON string trong Redis Hash
    pipeline.hset('products', p.id, JSON.stringify(p));
    // Lưu stock riêng để PU4 có thể DECR atomic
    pipeline.set(`stock:${p.id}`, p.stock);
  }

  await pipeline.exec();
  console.log(`✅ Seeded ${products.length} products into Data Grid`);
  console.log('   Keys: products (hash), stock:p1..p8 (string)');
  redis.disconnect();
}
